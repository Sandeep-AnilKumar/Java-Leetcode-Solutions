...
public class FraudulentUsersWithOrder {
	...
	public static List<String> fradulentUsers(List<String> transactions) {
		...
		int ListOfUsersSize = 0;
		String transactionWithListSize = "";
		int eventOccuredAt = 0;
		...
		for(Iterator<String> i = transactions.iterator(); i.hasNext();) {
			...
			if(transaction != null && transaction.length() != 0) {
				...
				if(!nameTransactionPair.containsKey(name)) {
					ListOfUsersSize = fraudulentUsers.size();
					...
					transactionWithListSize = transaction + "|" + ListOfUsersSize;
					nameTransactionPair.put(name,transactionWithListSize);
				}

				else if(amount > 3000 && !fraudulentUsers.contains(name)) {
					fraudulentUsers.add(name);
				}
					...
					if(!location.equals(oldLocation)) {
						...
						if(time - oldTransactionTime < 60 && !fraudulentUsers.contains(name)) {
							eventOccuredAt = Integer.parseInt(oldTransactionParts[4]);
							fraudulentUsers.add(eventOccuredAt,name);
						}
					}
					ListOfUsersSize = fraudulentUsers.size();
					transactionWithListSize = transaction + "|" + ListOfUsersSize;
