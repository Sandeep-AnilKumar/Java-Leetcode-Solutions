...
public class PacificAtlanticWaterFlow {
    public List<int[]> pacificAtlantic(int[][] matrix) {
        ...
        for (int j = 0; j < cols; ++j) {
            dfs(matrix, 0, j, list, visited, count, matrix[0][j], dir);
        }

        for (int i = 0; i < rows; ++i) {
            dfs(matrix, i, 0, list, visited, count, matrix[i][0], dir);
        }

        visited = new boolean[rows][cols];

        for (int j = 0; j < cols; ++j) {
            dfs(matrix, rows - 1, j, list, visited, count, matrix[rows - 1][j], dir);
        }

        for (int i = 0; i < rows; ++i) {
            dfs(matrix, i, cols - 1, list, visited, count, matrix[i][cols - 1], dir);
        }
        return list;
    }

    private void dfs(int[][] matrix, int i, int j, List<int[]> list, boolean[][] visited, int[][] count, int prev, int[][] dir) {
        if (i < 0 || j < 0 || i >= matrix.length || j >= matrix[i].length || matrix[i][j] < prev || visited[i][j]) return;
        ...
        for (int[] d : dir) {
            dfs(matrix, i + d[0], j + d[1], list, visited, count, matrix[i][j], dir);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        coordinates = pacificAtlanticWaterFlow.pacificAtlantic(matrix);
        for (int[] c : coordinates) {
            System.out.println(Arrays.toString(c));
        }

        System.out.println("============================================");
        matrix = new int[][] {{11,2,11,0,15,12,4,15,0,14,11,3,19,11,5,11,18,19,4,3,11,1,9,17,5,2,15,18,11,15},
